		...
		while (true) {
			try {
				...
				if (delivery.getEnvelope().getExchange().equals(rabbitExchange)) {
					// WE HAVE A DELIVERY!!!
					// We going to pass the message to another object and let it
					// handle
					// the parsing.

					String body = new String(delivery.getBody());
					String key = new String(delivery.getEnvelope()
							.getRoutingKey());
				...
				mqChannel.basicAck(delivery.getEnvelope().getDeliveryTag(),
						false);
	...
	private void connectRabbit() {
		...
		try {
			...
			mqChannel.exchangeDeclare(rabbitExchange, rabbitExchangeType,
					false, rabbitDurable, rabbitAutoDelete, null);

			// Add a random string to queue name so each instance of rocksteady
			// will
			// have its own queue
			Random r = new Random();
			...
			mqChannel.queueDeclare(rabbitQueue, false, rabbitDurable, false,
					rabbitAutoDelete, null);
			mqChannel.queueBind(rabbitQueue, rabbitExchange, rabbitRoutingKey);
			// mqChannel.queueBind(rabbitQueue, rabbitExchangeLog,
			// rabbitRoutingKey);
